...
<title>Admin Dashboard &mdash; Music Management</title>
...
<main class="main-content">
    <div class="page-header">
        <div>
            <h1 class="page-title">Manage artist</h1>
            <p class="page-sub">Register, edit, search and remove artist accounts</p>
        </div>
    </div>

    <?php if (isset($_GET['msg'])): ?>
        <?php $messages = ['added' => 'Artist added successfully.',
    ...
    <div class="card form-card">
        <h3 class="card-title">
            <?= $isEdit ? '&#9998; Edit Artist (#' . intval($editing['id']) . ')' : '+ Add New Artist' ?>
        </h3>
        <form method="POST"
    ...
    <div class="card">
        <div class="card-toolbar">
            ...
            <span class="badge" id="resultCount"><?= count($artist) ?> total</span>
        </div>
        ...
        <div class="table-wrap">
            <table class="data-table">
                ...
                <tbody id="tableBody">
                    <?php if (empty($artist)): ?>
                        <tr><td colspan="5" class="empty">No artist yet.</td></tr>
                    <?php else: ?>
                        <?php foreach ($artist as $i => $lib): ?>
                            <tr>
                                <td><?= $i + 1 ?></td>
...
(function () {
    ...
    input.addEventListener('input', function () {
        ...
        timer = setTimeout(function () {
            fetch('index.php?page=ajax&type=artist&q=' + encodeURIComponent(input.value.trim()),
